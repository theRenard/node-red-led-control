...
module.exports = function(numDevices) {

  var emitter = new events.EventEmitter;
  this.emitter = emitter;

  var FakeGpio = function() {}

  FakeGpio.prototype = new events.EventEmitter;
  ...
  FakeGpio.prototype.writeSync = function(value) {
	...
	var spiMosi= new FakeGpio('dataPin');
	var spiClk = new FakeGpio('clkPin');
	var spiCs  = new FakeGpio('csPin');
  ...
  spiMosi.on('writeSync', function() {
    emitter.emit('update', { dataPin: dataPin.value, clkPin: clkPin.value, csPin: csPin.value });
  });
  spiClk.on('writeSync', function() {
    emitter.emit('update', { dataPin: dataPin.value, clkPin: clkPin.value, csPin: csPin.value });
  });
  spiCs.on('writeSync', function() {
    emitter.emit('update', { dataPin: dataPin.value, clkPin: clkPin.value, csPin: csPin.value });
  });
